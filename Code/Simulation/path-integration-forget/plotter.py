        ...
        if cell_type in ['TN1', 'TN2']:
            ...
            if i % 2 == 0:
                l = ax[i].legend(handles=handles,
                                 ...
                                 handlelength=0,
                                 handletextpad=0)
                colors_flow = [flow_color_L, flow_color_R]
                for i, text in enumerate(l.get_texts()):
                    text.set_color(colors_flow[i])
                for handle in l.legendHandles:
                    handle.set_visible(False)
                l.draw_frame(False)
...
# ==========================
# This implementation uses the animation package to animate the plot and store it as mp4 file.
# It is very slow. 
# Based on https://matplotlib.org/gallery/animation/subplots.html
class SubplotAnimation(animation.TimedAnimation):
    def __init__(self, h, v, log, include=['TN1', 'TN2', 'CL1', 'TB1', 'CPU4', 'CPU1', 'motor'],
                fig=None, ax=None, colormap='viridis', title_x=-0.15,
                alpha=0.2, outbound_color='purple', return_color='g',
                label_font_size=11, unit_font_size=10, dashes=[1, 2, 1, 2],
                T_almost_home=None, t_start=0):
        
        self.T, self.T_outbound, self.T_inbound = log.T, log.T_outbound, log.T_inbound
        self.T_almost_home = T_almost_home
        titles = {'TN1': 'TN1 (Speed)', 'TN2': 'TN2 (Speed)', 'TL2': 'TL2',
                  'CL1': 'CL1', 'TB1': 'TB1 (Compass)', 'CPU4': 'CPU4 (Memory)',
                  'CPU1': 'CPU1 (Steering)', 'motor': 'motor', 'Route': 'Route'}
        self.data = {'TN1': log.tn1, 'TN2': log.tn2, 'TL2': log.tl2, 'CL1': log.cl1,
                'TB1': log.tb1, 'CPU4': log.cpu4, 'CPU1': log.cpu1,
                'motor': log.motor}

        self.colors = {'TL2': tl2_color, 'CL1': cl1_color}
        self.label_font_size = label_font_size
        
        self.t = np.array(range(self.T))
        self.include = include
        self.outbound_color = outbound_color
        ...
        # Generate new plot if no axes passed in.
        if ax is None:
            #fig, ax = plt.subplots(len(include), 1, figsize=(7, 7))
            fig = plt.figure(constrained_layout=True, figsize=(14, 7))
            ax = []
            if 'Route' in include:
                subplots_num_cols  = 2
                subplots_num_rows  = len(include) - 1
                subplots_num_left  = 1
                subplots_num_right = len(include) - 1
            else:
                subplots_num_cols  = 1
                subplots_num_rows  = len(include)
                subplots_num_left  = 0
                subplots_num_right = len(include)
            gs = gridspec.GridSpec(subplots_num_rows, subplots_num_cols, figure=fig)

            if 'Route' in include:
                ax.append(fig.add_subplot(gs[:, 0]))
            for i in range(subplots_num_rows):
                ax.append(fig.add_subplot(gs[i, subplots_num_cols-1]))
            
        self.ax = ax
        
        N_plots = len(include)
        
        self.line = [None] * N_plots
        ...
        for i, cell_type in enumerate(include):
            ...
            if cell_type in ['Route']:
                self.line[i] = Line2D([], [])
                ax[i].add_line(self.line[i])
                ax[i].set_xlim([np.min(self.x), np.max(self.x)])
                ax[i].set_ylim([np.min(self.y), np.max(self.y)])
            elif cell_type in ['TN1', 'TN2']:
                self.filtered_l = sp.ndimage.filters.gaussian_filter1d(self.data[cell_type][0], sigma=20)
                self.filtered_r = sp.ndimage.filters.gaussian_filter1d(self.data[cell_type][1], sigma=20)
                #tn_l_line, = ax[i].bar(['Left'], filtered_l[t], color=flow_color_L, label='L');
                #tn_r_line, = ax[i].bar(['Right'],filtered_r[t], color=flow_color_R, label='R');
                #handles = [tn_l_line, tn_r_line]

                #ax[i].bar(['Left'], data[cell_type][0][t], color=flow_color_L, alpha=0.3, lw=0.5);
                #ax[i].bar(['Right'],data[cell_type][1][t], color=flow_color_R, alpha=0.3, lw=0.5);
                #ax[i].set_yticks([0, 1])
                #ax[i].set_yticklabels([0, 1])
                self.line[i] = Line2D([], [], drawstyle='steps-mid', marker='o')
                #self.line[i].set_drawstyle('steps-mid')
                ax[i].add_line(self.line[i])
                ax[i].set_ylim([-1.05, 1.05])
                #ax[i].set_xlim([0, 4])
                ax[i].set_xticks([1, 4])
                ax[i].set_xticklabels([0, 5])

            elif cell_type in ['TL2', 'CL1'] and self.data[cell_type].shape[0] == 1:
                self.line[i] = Line2D([], [])
                ax[i].add_line(self.line[i])
                ax[i].set_yticks([-np.pi, np.pi])
                ax[i].set_yticklabels([0, 360])
                ...
                if cell_type == 'TB1':
                    #ax[i].set_xticks([1, 7])
                    #ax[i].set_xticklabels([1, 8])
                    ax[i].set_xlim([0, 7])
                    ax[i].set_xticks(range(0, 8))
                    ax[i].set_xticklabels(range(1, 8+1))
                else:
                    #ax[i].set_xticks([1, 14])
                    #ax[i].set_xticklabels([1, 16])
                    ax[i].set_xlim([0, 15])
                    ax[i].set_xticks(range(0, 16))
                    ax[i].set_xticklabels(range(1, 16+1))
                if cell_type == 'CPU4':
                    ax[i].hlines(0.5, 0, 15, linestyles='dashed', colors='grey')
            else:
                # Plots related to steering
                ax[i].set_ylim([-1.05, 1.05])
                self.line[i] = Line2D([], [], drawstyle='steps-mid', marker='o')
        ...
        # Add x labels to bottom plot
        #ax[N_plots-1].set_xlabel('Time (steps)', fontsize=label_font_size)
        #ax[N_plots-1].get_xaxis().set_major_formatter(mpl.ticker.FuncFormatter(lambda x, p: format(int(x), ',')))
        
        # Top spacer
        ax_space = fig.add_axes([0, 0.92, 1, 0.01])
        ax_space.axis('off')
        
        animation.TimedAnimation.__init__(self, fig, interval=5, blit=True, repeat=False)
    
    def _draw_frame(self, framedata):
        t_i = framedata
        t = self.t[t_i]
        
        N_plots = len(self.include)
        for i, cell_type in enumerate(self.include):
            if cell_type in ['Route']:
                # Outbound path
                self.line[i].set_data(self.x[0:t], self.y[0:t])
                
            elif cell_type in ['TN1', 'TN2']:
                y = [0, self.data[cell_type][0][t], 0, self.data[cell_type][1][t], 0]
                self.line[i].set_data(range(len(y)), y)
                #self.line[i].set_data()
                #tn_l_line, = ax[i].bar(['Left'], self.filtered_l[t], color=flow_color_L, label='L');
                #tn_r_line, = ax[i].bar(['Right'],self.filtered_r[t], color=flow_color_R, label='R');
                #handles = [tn_l_line, tn_r_line]
                ...
                y = self.data[cell_type][0]
                self.line[i].set_data(range(len(y)), y)
                #self.line[i].set_color(colors[cell_type])
            elif cell_type in ['TL2', 'CL1', 'TB1', 'CPU4', 'CPU1']:
                # Surface plots related to memory generation.
                y = self.data[cell_type][:,t]
                self.line[i].set_data(range(len(y)), y)
            else:
                # Plots related to steering
                y = self.data[cell_type][t]
                self.line[i].set_data([0, 1, 2], [0, y, 0])
                if t <= self.T_outbound:
                    self.line[i].set_color(self.outbound_color)
                else:
                    self.line[i].set_color(self.return_color)


            # State title
            labels = ['Outbound', 'Return', 'Search']
            if self.T_almost_home is None:
                self.T_almost_home = self.T_outbound + 400  # TODO(tomish) Auto generate this
            trajectory_stage = np.array([self.T_outbound, self.T_almost_home, self.T])
            if t <= self.T_outbound:
                stage = labels[0]
            elif t > self.T_almost_home:
                stage = labels[2]
            elif t > self.T_outbound:
                stage = labels[1]
            x_max, y_max = self.ax[0].get_ylim()
            self.ax[0].text(x_max, y_max*1.2, stage, fontsize=self.label_font_size, va='center', ha='center')
...
# This is a slow method for creating the plot animation that displays the result on screen
def plot_traces_animated(h, v, log, include=['TN1', 'TN2', 'CL1', 'TB1', 'CPU4', 'CPU1', 'motor'],
                fig=None, ax=None, colormap='viridis', title_x=-0.15,
                alpha=0.2, outbound_color='purple', return_color='g',
                label_font_size=11, unit_font_size=10, dashes=[1, 2, 1, 2],
                T_almost_home=None, t_start=0):
    ani = SubplotAnimation(h, v, log, include=include,
                fig=fig, ax=ax, colormap=colormap, title_x=title_x,
                alpha=alpha, outbound_color=outbound_color, return_color=return_color,
                label_font_size=label_font_size, unit_font_size=unit_font_size, dashes=dashes,
                T_almost_home=T_almost_home, t_start=t_start)
...
# This implementation updates the plot and instead of displaying it it stores it as png frames.
# Prefered. 
class SubplotAnimation2(animation.TimedAnimation):
    def __init__(self, h, v, log, include=['TN1', 'TN2', 'CL1', 'TB1', 'CPU4', 'CPU1', 'motor'],
                fig=None, ax=None, colormap='viridis', title_x=-0.15,
                alpha=0.2, outbound_color='purple', return_color='g',
                label_font_size=11, unit_font_size=10, dashes=[1, 2, 1, 2],
                T_almost_home=None, t_start=0, plot_speed=True, plot_heading=False):
        ...
        for i, cell_type in enumerate(include):
            ...
            if cell_type in ['Route']:
                ...
                #self.line[i].set_drawstyle('steps-mid')
                #ax[i].add_line(self.line[i])
                self.line[i], = ax[i].plot([], [], drawstyle='steps-mid')
                ax[i].set_ylim([-1.05, 1.05])
                ax[i].set_xlim([0, 4])
                ax[i].set_xticks([1, 3])
                ax[i].set_xticklabels(['Right', 'Left'])
    ...
    def draw_frame(self, t):
        ...
        for i, cell_type in enumerate(self.include):
            if cell_type in ['Route']:
                ...
                self.ax[i].cla()
                #self.line[i].remove()
                # When cleared it looses the axis ranges so set them again
                self.ax[i].set_xlim([np.min(self.x), np.max(self.x)])
                self.ax[i].set_ylim([np.min(self.y), np.max(self.y)])

                # Outbound path
                if self.plot_speed:
                    speed = np.clip(np.linalg.norm(np.vstack([np.diff(self.x), np.diff(self.y)]),
                                    axis=0), 0, 1)
                    n_min = np.argmin(speed[:self.T_outbound])
                    n_max = np.argmax(speed[:self.T_outbound])

                    for k in range(t+1):
                        self.ax[i].plot(self.x[k:k+2], self.y[k:k+2], color=(speed[k], 0.2, 1-speed[k]), lw=lw)

                else:
                    line_out, = self.ax[i].plot(self.x[0:t], self.y[0:t], lw=lw,
                                        color=self.outbound_color, label='Outbound')

                if self.plot_heading:
                    #interval = T/200  # Good for actual route plots thing (with headwidth 0)
                    interval = 20  # Good for memory plot thing (with headwidth 4)
                    self.ax[i].quiver(x[1:t:interval], y[1:t:interval],
                              np.sin(h[1:t:interval]),
                              np.cos(h[1:t:interval]),
                              pivot='tail', width=0.003, scale=12.0, headwidth=4, color=quiver_color)
                              #pivot='tail', width=0.002, scale=12.0, color=quiver_color)

                # Inbound path
                line_in, = self.ax[i].plot(self.x[self.T_outbound:t], self.y[self.T_outbound:t], color=self.return_color,
                                   lw=lw, label='Return')
            ...
            if t <= self.T_outbound:
                ...
                stage = labels[1]
            x_max, y_max = self.ax[0].get_ylim()
            self.ax[0].text(x_max, y_max+2, stage, fontsize=self.label_font_size, va='center', ha='center', bbox=dict(facecolor='white', alpha=1.0, edgecolor='none', pad=0))
        
            self._drawn_artists = self.line
            
            # It is slow with this method
            #self.fig.canvas.draw()
            # This is a suggested alternative that was not tested
            #self.fig.canvas.draw_idle()
            # It is faster with this method
            # Copy the image to the GUI state, but screen might not changed yet
            self.fig.canvas.blit(self.ax[i].bbox)
            # Flush any pending GUI events, re-painting the screen if needed
            self.fig.canvas.flush_events()
    
    # This is called to start saving frames
    def play(self, filename):
        for t in range(self.t.size):
            self.draw_frame(t)
            if filename is not None:
                self.fig.savefig(filename.format(t))

# This is a faster method for creating the plot animation that does not display the result on screen
def plot_traces_animated2(h, v, log, include=['TN1', 'TN2', 'CL1', 'TB1', 'CPU4', 'CPU1', 'motor'],
                fig=None, ax=None, colormap='viridis', title_x=-0.15,
                alpha=0.2, outbound_color='purple', return_color='g',
                label_font_size=11, unit_font_size=10, dashes=[1, 2, 1, 2],
                T_almost_home=None, t_start=0, filename=None):
    anim = SubplotAnimation2(h, v, log, include=include,
                fig=fig, ax=ax, colormap=colormap, title_x=title_x,
                alpha=alpha, outbound_color=outbound_color, return_color=return_color,
                label_font_size=label_font_size, unit_font_size=unit_font_size, dashes=dashes,
                T_almost_home=T_almost_home, t_start=t_start)
...
def plot_traces_animated1(log, include=['TN1', 'TN2', 'CL1', 'TB1', 'CPU4', 'CPU1', 'motor'],
                fig=None, ax=None, colormap='viridis', title_x=-0.15,
                alpha=0.2, outbound_color='purple', return_color='g',
                label_font_size=11, unit_font_size=10, dashes=[1, 2, 1, 2],
                T_almost_home=None, t_start=0):
    ...
    for i, cell_type in enumerate(include):
        ...
        if cell_type in ['TN1', 'TN2']:
            filtered_l = sp.ndimage.filters.gaussian_filter1d(data[cell_type][0], sigma=20)
            filtered_r = sp.ndimage.filters.gaussian_filter1d(data[cell_type][1], sigma=20)
            tn_l_line, = ax[i].bar(['Left'], filtered_l[t], color=flow_color_L, label='L');
            tn_r_line, = ax[i].bar(['Right'],filtered_r[t], color=flow_color_R, label='R');
            handles = [tn_l_line, tn_r_line]
            ...
            ax[i].set_ylim([0, 1.05])
            if t <= T_outbound:
                ax[i].bar(['Motor'], data[cell_type][t], color=outbound_color)
            else:
                ax[i].bar(['Motor'], data[cell_type][t], color=return_color)
        ...
        if T_almost_home is None:
            T_almost_home = T_outbound + 400  # TODO(tomish) Auto generate this
        trajectory_stage = np.array([T_outbound, T_almost_home, T])
        if t <= T_outbound:
            stage = labels[0]
        elif t > T_almost_home:
            stage = labels[2]
        elif t > T_outbound:
            stage = labels[1]
        x_max, y_max = ax[0].get_ylim()
        ax[0].text(x_max, y_max*1.2, stage, fontsize=label_font_size, va='center', ha='center')
