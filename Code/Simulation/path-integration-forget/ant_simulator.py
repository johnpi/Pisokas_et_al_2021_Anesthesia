# This is an alternative to the bee_simulator.py. 
# This script better immitates the dynamics of the ants' motion.
...
def get_next_state(heading, velocity, rotation, acceleration, drag=0.5, noise_turn=0.0):
    """Get new heading and velocity, based on relative rotation and
    acceleration and linear drag."""
    rotation = rotation * np.random.normal(loc=1.0, scale=noise_turn) # Noise as a function of memory amplitude. 

    theta = rotate(heading, rotation)
    v = velocity + thrust(theta, acceleration)
    v -= drag * v
    # If turning more than 20deg stop
    if rotation > 0.3491 or rotation < -0.3491: # rotation > 20deg
        v = 0.0
    return theta, v
